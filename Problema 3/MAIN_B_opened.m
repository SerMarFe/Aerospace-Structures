...
% b.1)
figure('Name','Opened section');
t = tiledlayout(3,1);
title(t,'\textbf{Opened section}','interpreter','latex')

% Vertical deflection (opened section)
ax1 = nexttile;
...
% Bending rotation (opened section)
ax2 = nexttile;
...
% Torsion rotation (opened section)
ax3 = nexttile;
...
[discretization_factor, tip_deflection_error, tip_torsion_error] = getTipDeflectoinAndTorsionError(512);
...
title("\textbf{Study of Convergence (opened)}",'interpreter','latex');
...
for i=1:data.nel
    [~,tau,~] = getTangentialStressDistribution(section_data.x_disc,section_data.Tn,section_data.m,section_data.Tm,section_data.Xc,section_data.Ixx,section_data.Iyy,section_data.Ixy,0,S_el(1,i));
    [tau_t,~] = getTangentialStressDistributionTorsion(section_data.x_disc,section_data.Tn,section_data.m,section_data.Tm,section_data.J,-Mt_el(1,i));
...
title(sprintf("Most demanded node (opened, section at %.2f m)",position_wing_max_sigma_vm));
