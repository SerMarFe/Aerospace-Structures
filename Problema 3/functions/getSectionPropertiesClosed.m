function [Xc,Xs,Atot,Ixx,Iyy,Ixy,J,Ain] = getSectionPropertiesClosed(x,Tn,m,Tm)
    ...
    for e = 1:nel
        ...
        r_cross_t_vec = r_cross_t_vec + cross([X_e(e,:)-Xc, 0],[dX 0]); % aproximació numèrica de la integral del producte vectorial entre el vector posició dels nodes respecte el centroide amb el vector tangent a aquests (per calcular l'àrea interior).
    end
    Ain = sqrt(sum(r_cross_t_vec.^2))/2;
    J = 4*Ain^2/J;
